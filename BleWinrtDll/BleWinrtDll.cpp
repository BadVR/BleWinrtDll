...
IAsyncOperation<BluetoothLEDevice> retrieveDevice(const wchar_t* deviceId) {
	...
	if (result == nullptr) {
		...
		try {
			clearError();
			co_await winrt::resume_after(chrono::seconds(1));
			{
				lock_guard lock(cacheLock);
...
{
	//read mac address
	uint64_t mac = eventArgs.BluetoothAddress();

	//read bluetooth advertisement
	BluetoothLEAdvertisement ad = eventArgs.Advertisement();
	int16_t rssi = eventArgs.RawSignalStrengthInDBm();
	uint32_t num_services = ad.ServiceUuids().Size();

	Log(L"\nScan received " + std::to_wstring(mac));
	
	Log(L"RSSI: " + std::to_wstring(rssi));

	//build a carrier for hte device update to return
	DeviceUpdate deviceUpdate;
	deviceUpdate.mac = mac;
	deviceUpdate.rssi = rssi;

	if (num_services > 0)
	{
		int index = 0;

		//extract each service and convert to a byte array
		Log(L"services: " + std::to_wstring(num_services));

		for (const winrt::guid& guid : ad.ServiceUuids())
		{
			const uint8_t* bytes = reinterpret_cast<const uint8_t*>(&guid);
			std::copy(bytes, bytes + sizeof(guid), deviceUpdate.services + index);
			index++;
		}
	}

	IReference<int16_t> transmitPowerLevel = eventArgs.TransmitPowerLevelInDBm();
	if (transmitPowerLevel)
	{
		short tx = transmitPowerLevel.Value();

		deviceUpdate.tx = tx;

		Log(L"TX: " + std::to_wstring(tx));
	}

	const auto nameLen = sizeof(deviceUpdate.name) / sizeof(wchar_t);
	wcscpy_s(deviceUpdate.name, nameLen, ad.LocalName().c_str());

	auto manData = ad.GetManufacturerDataByCompanyId(0xFFFF);
	...
	auto dev = co_await BluetoothLEDevice::FromBluetoothAddressAsync(mac);
	if (dev)
	{
		Log("got dev");
		DeviceInformation info = dev.DeviceInformation();

		auto pairing = info.Pairing();

		if (pairing)
			deviceUpdate.isConnectable = pairing.CanPair();
	}
	/*
...
winrt::fire_and_forget ScanServicesAsync(wchar_t* deviceId) {
	...
	Log(L"Scanning services of " + wstring(deviceId));
	try {
		const auto bluetoothLeDevice = co_await retrieveDevice(deviceId);
		co_await winrt::resume_after(chrono::milliseconds(1)); // Never too much waiting with ble
...
winrt::fire_and_forget ScanCharacteristicsAsync(unique_ptr<wstring> deviceId, unique_ptr<wstring> serviceId) {
	...
	try {
		...
		co_await winrt::resume_after(chrono::milliseconds(100));
		if (service != nullptr) {
