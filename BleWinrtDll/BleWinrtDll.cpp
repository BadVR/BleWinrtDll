...
winrt::hstring CreateServiceIdAqs(const wchar_t* serviceUuid)
{
	std::wstring aqs = L"(System.DeviceInterface.Bluetooth.ServiceGuid:=\"" + std::wstring(serviceUuid) + L"\")";
	return winrt::hstring(aqs);
}

void StartDeviceScanServices(wchar_t* requiredServices[], std::uint32_t serviceCount)
{
	// as this is the first function that must be called, if Quit() was called before, assume here that the client wants to restart
	{
		lock_guard lock(quitLock);
	...
	if (serviceCount > 0)
	{
		std::vector<winrt::hstring> aqsFilters;
		aqsFilters.reserve(serviceCount);

		for (size_t i = 0; i < serviceCount; ++i)
		{
			winrt::hstring aqs = CreateServiceIdAqs(requiredServices[i]);
			aqsFilters.push_back(aqs);
		}

		aqsFilter = L"(";
		for (size_t i = 0; i < aqsFilters.size(); ++i)
		{
			aqsFilter = aqsFilter + aqsFilters[i];
			if (i < aqsFilters.size() - 1)
				aqsFilter = aqsFilter + L" OR ";
		}
		aqsFilter = aqsFilter + L")";
	}
	else
	{
		// No specific service UUIDs provided, get all Bluetooth LE devices
		aqsFilter = L"(System.Devices.Aep.ProtocolId:=\"{bb7bb05e-5972-42b5-94fc-76eaa7084d49}\")";
	}
	*/
	/*
	std::vector<winrt::hstring> aqsFilters;
	...
	*/
	aqsFilter = L"(System.Devices.Aep.ProtocolId:=\"{bb7bb05e-5972-42b5-94fc-76eaa7084d49}\") AND (System.DeviceInterface.Bluetooth.ServiceGuid:=\"{f29b1525-cb19-40f3-be5c-7241ecb82fd1}\")";
